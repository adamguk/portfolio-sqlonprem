...
SELECT p.BusinessEntityID AS PersonNK,
       p.PersonType,
       CASE p.PersonType WHEN 'SC' THEN 'Store Contact' WHEN 'IN' THEN 'Individual' WHEN 'SP' THEN 'Salesperson' WHEN 'EM' THEN 'Employee Non-Sales' WHEN 'VC' THEN 'Vendor Contact' WHEN 'GC' THEN 'General Contact' ELSE 'NA' END AS PersonTypeDescription,
       CASE p.PersonType WHEN 'SP' THEN 'Internal Employees' WHEN 'EM' THEN 'Internal Employees' ELSE 'NA' END AS PersonTypeGroup,
       ...
       HASHBYTES('SHA2_256', CONCAT_WS('|', COALESCE (p.PersonType, ''), CASE p.PersonType WHEN 'SC' THEN 'Store Contact' WHEN 'IN' THEN 'Individual' WHEN 'SP' THEN 'Salesperson' WHEN 'EM' THEN 'Employee Non-Sales' WHEN 'VC' THEN 'Vendor Contact' WHEN 'GC' THEN 'General Contact' ELSE 'NA' END, CASE p.PersonType WHEN 'SP' THEN 'Internal Employees' WHEN 'EM' THEN 'Internal Employees' ELSE 'NA' END, COALESCE (p.Title, ''), COALESCE (p.FirstName, ''), COALESCE (p.MiddleName, ''), COALESCE (p.LastName, ''), COALESCE (p.Suffix, ''), COALESCE (e.EmailAddress, ''), CAST (COALESCE (p.EmailPromotion, 0) AS VARCHAR (10)))) AS RowHash
FROM   STG.Person_Person AS p
       LEFT OUTER JOIN
       INT.Person_EmailAddress AS e
...
AS
SELECT A.AddressID AS PersonAddressNK,
       A.AddressLine1,
       A.AddressLine2,
       A.City,
       A.StateProvinceID AS StateProvinceNK,
       A.PostalCode,
       A.SpatialLocation,
       A.ModifiedDate,
       A.ExtractDatetime,
       A.RowHash
FROM   STG.Person_Address AS A;


GO
CREATE OR ALTER VIEW INT.Person_AddressType
AS
SELECT AddressTypeID AS PersonAddressTypeNK,
...
AS
SELECT --Keys
       a.PersonAddressNK AS AddressNK,
       ...
       HASHBYTES('SHA2_256', CONCAT_WS('|', COALESCE (a.AddressLine1, 'NA'), COALESCE (a.AddressLine2, 'NA'), COALESCE (a.City, 'NA'), COALESCE (a.PostalCode, 'NA'), COALESCE (sp.StateProvinceCode, 'NA'), COALESCE (sp.StateProvinceName, 'NA'), COALESCE (cr.CountryRegionName, 'NA'))) AS RowHash
FROM   INT.Person_Address AS a
       LEFT OUTER JOIN
       INT.Person_StateProvince AS sp
       ON a.StateProvinceNK = sp.StateProvinceNK
       LEFT OUTER JOIN
       INT.Person_CountryRegion AS cr
...
FROM   INT.Person_Person AS p
       ...
       INNER JOIN
       INT.Person_Address AS a
       ON bea.PersonAddressNK = a.PersonAddressNK
       INNER JOIN
       INT.Person_AddressType AS t
...
AS
SELECT p.ProductID AS ProductNK,
       ...
       p.ExtractDatetime,
       --TYPE 2 (versioned) tracked attributes
       HASHBYTES('SHA2_256', CONCAT_WS('|', COALESCE (p.Name, 'NA'), CAST (p.MakeFlag AS VARCHAR (10)), IIF (p.MakeFlag = 0, 'Vendor Supplied', 'Made In-House'), CAST (p.FinishedGoodsFlag AS VARCHAR (10)), IIF (p.FinishedGoodsFlag = 0, 'Production Component', 'Consumer Product'), COALESCE (p.Color, 'NA'), CONVERT (NVARCHAR (50), p.StandardCost, 2), CONVERT (NVARCHAR (50), p.ListPrice, 2), CAST (p.Size AS VARCHAR (50)), COALESCE (TRIM(p.SizeUnitMeasureCode), 'NA'), COALESCE (TRIM(p.WeightUnitMeasureCode), 'NA'), CAST (p.Weight AS VARCHAR (50)), COALESCE (TRIM(p.ProductLine), 'NA'), COALESCE (TRIM(p.Class), 'NA'), COALESCE (TRIM(p.Style), 'NA'), CAST (COALESCE (pc.ProductCategoryID, -1) AS VARCHAR (50)), COALESCE (pc.Name, 'NA'), CAST (COALESCE (p.ProductSubcategoryID, -1) AS VARCHAR (50)), COALESCE (sub.Name, 'NA'), CAST (COALESCE (p.ProductModelID, -1) AS VARCHAR (50)), COALESCE (mod.Name, 'NA'), COALESCE (CONVERT (NVARCHAR (30), p.SellStartDate, 126), 'NA'), COALESCE (CONVERT (NVARCHAR (30), p.SellEndDate, 126), 'NA'), COALESCE (CONVERT (NVARCHAR (30), p.DiscontinuedDate, 126), 'NA'))) AS RowHash
FROM   STG.Production_Product AS p
       LEFT OUTER JOIN
       STG.Production_ProductSubcategory AS sub
       ON p.ProductSubcategoryID = sub.ProductSubcategoryID
       LEFT OUTER JOIN
       STG.Production_ProductCategory AS pc
       ON sub.ProductCategoryID = pc.ProductCategoryID
       LEFT OUTER JOIN
       STG.Production_ProductModel AS mod
...
SELECT --Keys
       ...
       ExtractDatetime,
       --Change detection
       HASHBYTES('SHA2_256', CONCAT_WS('|', COALESCE ([Name], 'NA'), COALESCE (CONVERT (NVARCHAR (50), ShipBase, 2), 'NA'), COALESCE (CONVERT (NVARCHAR (50), ShipRate, 2), 'NA'))) AS RowHash
...
AS
SELECT [SalesOrderID] AS SalesOrderNK,
...
SELECT [SalesOrderID] AS SalesOrderNK,
       ...
       CASE [Status] WHEN 1 THEN 'In process' WHEN 2 THEN 'Approved' WHEN 3 THEN 'Backordered' WHEN 4 THEN 'Rejected' WHEN 5 THEN 'Shipped' WHEN 6 THEN 'Cancelled' ELSE 'No Status' END AS StatusDescription,
       [OnlineOrderFlag],
...
SELECT --Keys
       ...
       ExtractDatetime,
       --Change detection
       HASHBYTES('SHA2_256', CONCAT_WS('|', COALESCE (CardType, 'NA'), COALESCE (CardNumber, 'NA'), CAST (COALESCE (ExpMonth, -1) AS NVARCHAR (50)), CAST (COALESCE (ExpYear, -1) AS NVARCHAR (50)))) AS RowHash
...
SELECT --Keys
       ...
       --Change detection
       HASHBYTES('SHA2_256', CONCAT_WS('|', COALESCE (CONVERT (NVARCHAR (50), rate.AverageRate, 2), 'NA'), COALESCE (CONVERT (NVARCHAR (50), rate.EndOfDayRate, 2), 'NA'))) AS RowHash
FROM   STG.Sales_CurrencyRate AS rate
       LEFT OUTER JOIN
       INT.Sales_Currency AS currF
       ON rate.FromCurrencyCode = currF.CurrencyCode
       LEFT OUTER JOIN
       INT.Sales_Currency AS currT
...
SELECT --Keys
       ...
       [ExtractDatetime],
       --Change detection
       HASHBYTES('SHA2_256', CONCAT_WS('|', COALESCE ([Description], 'NA'), CONVERT (NVARCHAR (50), COALESCE ([DiscountPct], 0), 2), COALESCE ([Type], 'NA'), COALESCE ([Category], 'NA'), COALESCE (CONVERT (NVARCHAR (30), [StartDate], 126), 'NA'), COALESCE (CONVERT (NVARCHAR (30), [EndDate], 126), 'NA'), COALESCE (CAST ([MinQty] AS VARCHAR (50)), 'NA'), COALESCE (CAST ([MaxQty] AS VARCHAR (50)), 'NA'))) AS RowHash
...
SELECT --Keys
       ...
       s.ExtractDatetime,
       --Change detection
       HASHBYTES('SHA2_256', CONCAT_WS('|', COALESCE (s.Name, 'NA'), CAST (COALESCE (s.SalesPersonID, -1) AS VARCHAR (10)), COALESCE (s.Demographics.value('(BusinessType)[1]', 'NVARCHAR(5)'), 'NA'), COALESCE (s.Demographics.value('(Specialty)[1]', 'NVARCHAR(50)'), 'NA'))) AS RowHash
...
FROM   STG.Sales_SalesPerson AS s;
...
SELECT --Keys
       ...
       st.ExtractDatetime,
       --Change detection
       HASHBYTES('SHA2_256', CONCAT_WS('|', COALESCE (st.[Name], 'NA'), COALESCE (st.CountryRegionCode, 'NA'), COALESCE (st.[Group], 'NA'))) AS RowHash
FROM   STG.Sales_SalesTerritory AS st;
...
SELECT --Keys
       ...
       --Shared Fields
       CASE WHEN c.PersonID IS NOT NULL THEN 'Individual' WHEN c.StoreID IS NOT NULL THEN 'Store' ELSE 'NA' END AS CustomerType,
       ...
       st.EmployeeCount AS Store_EmployeeCount,
       ...
       c.ExtractDatetime,
       --Change detection
       HASHBYTES('SHA2_256', CONCAT_WS('|', CASE WHEN c.PersonID IS NOT NULL THEN 'Individual' WHEN c.StoreID IS NOT NULL THEN 'Store' ELSE 'NA' END, COALESCE (c.AccountNumber, 'NA'), COALESCE (st.StoreName, 'NA'), COALESCE (CONVERT (NVARCHAR (50), st.AnnualRevenue, 2), 'NA'), COALESCE (CONVERT (NVARCHAR (50), st.AnnualSales, 2), 'NA'), COALESCE (st.BusinessType, 'NA'), COALESCE (st.Specialty, 'NA'), COALESCE (CAST (st.YearOpened AS NVARCHAR (10)), 'NA'), COALESCE (CAST (st.EmployeeCount AS NVARCHAR (50)), 'NA'), COALESCE (p.PersonTypeDescription, 'NA'), COALESCE (p.FullName, 'NA'), COALESCE (p.EmailAddress, 'NA'), COALESCE (p.EmailPromotionSignUp, 'NA'))) AS RowHash
FROM   STG.Sales_Customer AS c
       LEFT OUTER JOIN
       INT.Sales_Store AS st
       ON c.StoreID = st.StoreNK
       LEFT OUTER JOIN
       INT.Person_Person AS p
...
SELECT --Keys
       ...
       --Change detection
       HASHBYTES('SHA2_256', CONCAT_WS('|', CONVERT (NVARCHAR (64), h.SalesOrderHeaderHash, 2), CONVERT (NVARCHAR (64), d.SalesOrderDetailHash, 2))) AS FactSalesHash
FROM   INT.Sales_SalesOrderDetail AS d
       INNER JOIN
       INT.Sales_SalesOrderHeader AS h
